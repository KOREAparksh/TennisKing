...
client.interceptors.response.use(
    ...
    (err) => {
        if (err.response.status === 401 && err.response.data.message !== "Invalid Code") window.location.href = "/login";
        return Promise.reject(err);
    }
);
...
export const postReserves = async (open_time, place_id, login, password, member, reserve_times, use_facility) => {
