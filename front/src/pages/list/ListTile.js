...
function ListTile({ id, login, openTime, place, reserveTimes, status }) {
    ...
    const findStatus = (e) => {
        ...
        reserveTimes.find(function (data) {
            if (data.time === e) answer = data.status;
        });
        return answer;
    };
    ...
    const getTimeTables = () => {
        ...
        reserveTimes.map(function (e) {
            ...
            if (typeof arr === "object") date_set.set(getDateCode(e.time), [...arr, e.time]);
            else date_set.set(getDateCode(e.time), [e.time]);
        });
        return [...date_set].map(function (e) {
            return (
                <Row key={e.id} className="flex-row-reverse text-end mt-1">
                    <div className=" fw-bold px-0">{new Date(+e[1][0] + 3240 * 10000).toISOString().slice(0, 10)}</div>
                    <div>
                        {e[1].map((e) => (
                            <div>
    ...
    return (
        <Row
            className="mt-5 mb-4 p-1 g-0 border border-dark text-responsive justify-content-center"
            style={{ backgroundColor: "bisque", borderRadius: "1rem" }}
        >
            <Row className={"fw-bold flex-row-reverse"}>{login} 님</Row>
            ...
            <div className="mb-2 btn-group" style={{ maxWidth: "250px" }} hidden={status === 0 ? false : true}>
                <Button variant="outline-danger mx-3" size="sm" className="rounded-pill" onClick={onClickDelete}>
                    삭제
                </Button>
            </div>
        </Row>
